...
def get_rais_columns(year: int, dataset: str) -> list[str]:
    if dataset == "vinculos":
...
def get_caged_columns(date: int, dataset: str) -> tuple[list[str], str]:
    if dataset == "caged":
        encoding = "latin-1"
...
def read_caged(
    ...
    columns_names, encoding = get_caged_columns(date, dataset)
